...
function parseRoutesFromFile(content, basePath) {
  ...
  const routeRegex = /router\.(get|post|put|patch|delete)\s*\(\s*['"`]([^'"`]+)['"`]([\s\S]*?(\w+Controller)\.([\w]+))/g;
  ...
  while ((match = routeRegex.exec(cleanContent)) !== null) {
    ...
    const isProtected = routeMw.hasProtect
      || (globalMw.hasGlobalProtect && routeLineIndex > globalMw.globalProtectLine);
    const isAdminOnly = routeMw.hasAuthorize
      || (globalMw.hasGlobalAuthorize && routeLineIndex > globalMw.globalAuthorizeLine);
...
function buildFieldsSubsetSchema(schemaKey, fieldNames) {
  ...
  for (const fieldName of fieldNames) {
    ...
    switch (rule.type) {
      ...
      case 'enum': prop.type = 'string'; prop.enum = rule.enum; break;
...
function buildOperation({ method, routePath, basePath, jsdoc, isProtected, isAdminOnly, adminRole, routeMw }) {
  ...
  if (method === 'get' && !routePath.includes(':') && routePath !== '/schema' && !routePath.includes('/stats') && routePath !== '/template' && routePath !== '/export') {
