...
function buildFieldsSubsetSchema(schemaKey: string, fieldNames: string[]) {
  const schema = (schemas[schemaKey.endsWith('s') ? schemaKey : schemaKey + 's'] || schemas[schemaKey]) as
    | SchemaDefinition
    | undefined;
...
function buildRequestBody(method: string, basePath: string, routeMw: RouteMiddlewareInfo) {
