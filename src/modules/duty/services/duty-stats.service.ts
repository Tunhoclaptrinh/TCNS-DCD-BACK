  ...
  async getComprehensiveStats(options: any = {}) {
    ...
    const users = allUsers.filter((u) => {
      if (
        ...
        return false;
      if (generationId && String(u.generationId) !== String(generationId)) return false;
      return true;
    });
    ...
    const stats = users.map((user) => {
      ...
      // Get user's base quota (per week)
      ...
      const userQuota = Number((baseQuota * numWeeks).toFixed(2));
      ...
      // Calculate total kips (using coefficient from kips)
